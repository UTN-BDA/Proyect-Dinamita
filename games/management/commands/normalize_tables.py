...
def main(input_path, output_dir):
    ...
    about_game = df[
        ["app_id", "detailed_description", "about_the_game", "short_description"]
    ]
    ...
    # Explode languages
    languages = (
        df[["app_id", "supported_languages"]]
        .explode("supported_languages")
        .rename(columns={"supported_languages": "language"})
    )
    audio_languages = (
        df[["app_id", "full_audio_languages"]]
        .explode("full_audio_languages")
        .rename(columns={"full_audio_languages": "audio_language"})
    )
    ...
    # Simple lists
    developers = (
        df[["app_id", "developers"]]
        .explode("developers")
        .rename(columns={"developers": "developer"})
    )
    publishers = (
        df[["app_id", "publishers"]]
        .explode("publishers")
        .rename(columns={"publishers": "publisher"})
    )
    categories = (
        df[["app_id", "categories"]]
        .explode("categories")
        .rename(columns={"categories": "category"})
    )
    genres = (
        df[["app_id", "genres"]].explode("genres").rename(columns={"genres": "genre"})
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Normaliza JSON objeto en tablas CSV separadas"
    )
    parser.add_argument("--input", required=True, help="Ruta al JSON objeto limpio")
    parser.add_argument(
        "--output_dir", required=True, help="Directorio de salida para tablas"
    )
